...
contract GasTankTest is Test {
    ...
    //     vm.expectRevert(IGasTank.InvalidOriginMessage.selector);
    //     gasTank.claim(id, address(this), payload);
    // }

    function testClaim_Success(uint256 baseFee, uint256 relayCost, bytes32 msgHash, bytes32 rootMsgHash) external {
        // vm.assume(msgHash != rootMsgHash);
        uint256 maxDeposit = gasTank.MAX_DEPOSIT();
        ...
        bytes memory payload = abi.encode(  // TODO TODO double encode and pack
            IGasTank.RelayedMessageGasReceipt.selector,  // log selector
            rootMsgHash,  // originMsgHash
            address(this),  // relayer
            relayCost,  // gasCost
        ...
        vm.expectEmit(address(gasTank));
        emit IGasTank.Claimed(rootMsgHash, address(this), address(this), totalCost);
        gasTank.claim(id, address(this), payload);

        assertEq(gasTank.balanceOf(address(this)), 0, "GasTank balance should be 0");
        assertTrue(gasTank.claimed(rootMsgHash), "GasTank should have claimed the root message");
    ...
    function testRelayMessage_Success() public {
        ...
        //     address(Predeploys.L2_TO_L2_CROSS_DOMAIN_MESSENGER),
        //     abi.encodeWithSignature("messageNonce()"),
        //     abi.encode(uint240(10), uint240(11))
        // );
        // vm.expectCall(
        //     address(Predeploys.L2_TO_L2_CROSS_DOMAIN_MESSENGER),
        ...
        //     address(Predeploys.L2_TO_L2_CROSS_DOMAIN_MESSENGER),
        //     abi.encodeWithSignature("messageNonce()"),
        //     abi.encode(uint240(11), uint240(12))
        // );
        // vm.expectCall(
        //     address(Predeploys.L2_TO_L2_CROSS_DOMAIN_MESSENGER),
