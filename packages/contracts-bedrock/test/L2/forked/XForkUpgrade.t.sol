...
contract XForkUpgradeTest is CommonTest {
    ...
    ///      complete.
    function test_setXForkUpgrade_feeVaults() external {
        vm.skip(!isL2ForkTest());

        /// 1. Deploy the new L1Block implementation contract
        /// 2. Upgrade the L1Block contract
        _upgradeL1Block();
        assertEq(IL1Block(Predeploys.L1_BLOCK_ATTRIBUTES).isXFork(), false);

        /// 3. Perform the isthmus upgrade in the L1Block contract
        vm.prank(Constants.DEPOSITOR_ACCOUNT);
        IL1Block(Predeploys.L1_BLOCK_ATTRIBUTES).setXFork();

        // Assert the isthmus upgrade was successful
        assertEq(IL1Block(Predeploys.L1_BLOCK_ATTRIBUTES).isXFork(), true);
    ...
    ///      is complete.
    function test_setXForkUpgrade_otherContracts() external {
        vm.skip(!isL2ForkTest());
