...
| 4   | SuperchainERC20 | Relaying `SuperchainWETH` sent from origin through `SuperchainTokenBrdige` on destination decreases the `ETHLiquidity` Ether balance, and increases the target’s `SuperchainWETH` balance on destination as well as `SuperchainWETH` Ether balance by exactly the input amount.                     |  [ ]   |
