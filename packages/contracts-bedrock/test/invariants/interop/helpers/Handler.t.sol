...
contract Handler is Setup, Actors {
    ...
    function handler_superchainWETHSendETH(address _to, uint256 _value, uint256 _chainId, uint256 _actorIndex) public {
        ...
        // TODO: Add check so it doesn't fail due to an ether overflow on superchain weth
        ...
        console.log("actor balance before: ", _actorBalanceBefore);
        console.log("value               : ", _value);
        console.log("sweth balance beforte: ", address(SUPER_WETH).balance);
        ...
        try actor.directCall(address(SUPER_WETH), _value, abi.encodeCall(SUPER_WETH.sendETH, (_to, _chainId))) {
            ...
            console.log("1");
            console.log("actor balance after: ", address(actor).balance);
            ...
            assert(false);
            // assert(_value > _actorBalanceBefore);
        }
    }
