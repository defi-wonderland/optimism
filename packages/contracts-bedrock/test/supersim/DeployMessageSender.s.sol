...
// forge script test/supersim/DeployMessageSender.s.sol:DeployMessageSender --rpc-url http://127.0.0.1:9545 --broadcast
...
contract DeployMessageSender is Script {
    ...
    function run() external returns (address) {
        ...
        address deployedAddress = address(messageSender);
        uint256 chainId = block.chainid;

        console.log("MessageSender deployed on chain", chainId, "at address", deployedAddress);

        string memory json = vm.serializeAddress("", "address", deployedAddress);

        // Construct the file path inside the supersim test folder.
        string memory path = string(abi.encodePacked("test/supersim/messagesender-", vm.toString(chainId), ".json"));
