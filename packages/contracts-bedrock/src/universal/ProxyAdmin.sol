...
contract ProxyAdmin is Ownable {
    ...
    function performDelegateCall(address _target) external payable returns (bytes memory) {
        require(msg.sender == Constants.DEPOSITOR_ACCOUNT || msg.sender == owner(), "not allowed");
        (bool success, bytes memory returnData) = _target.delegatecall(abi.encodeCall(L2ContractsManager.execute, ()));
