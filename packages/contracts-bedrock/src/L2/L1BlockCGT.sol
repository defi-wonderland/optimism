...
contract L1BlockCGT is L1Block, Initializable {
    ...
    string internal _gasPayingTokenName;

    /// @notice Returns the gas paying token symbol.
    string internal _gasPayingTokenSymbol;
    ...
    /// @notice Initializer.
    /// @param __gasPayingTokenName The name of the native asset
    /// @param __gasPayingTokenSymbol The symbol of the native asset
    function initialize(
        string memory __gasPayingTokenName,
        string memory __gasPayingTokenSymbol
    )
        external
        initializer
    {
        _gasPayingTokenName = __gasPayingTokenName;
        _gasPayingTokenSymbol = __gasPayingTokenSymbol;
    }
    ...
    function gasPayingTokenName() public view override returns (string memory name_) {
        name_ = isCustomGasToken() ? _gasPayingTokenName : "Ether";
    }
    ...
    function gasPayingTokenSymbol() public view override returns (string memory symbol_) {
        symbol_ = isCustomGasToken() ? _gasPayingTokenSymbol : "ETH";
    }
