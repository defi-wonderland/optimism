...
import {XSuperchainERC20} from './XSuperchainERC20.sol';
...
contract XSuperchainERC20Factory is IXERC20Factory {
  using EnumerableSet for EnumerableSet.AddressSet;

  /**
   * @notice Address of the xsuperc20 maps to the address of its lockbox if it has one
   */
  ...
  /**
   * @notice The set of registered xSupERC20 tokens
   */
  EnumerableSet.AddressSet internal _xerc20RegistryArray;

  /**
   * @notice Deploys an xSupERC20 contract using CREATE3
   ...
   * @return _xerc20 The address of the xsuperc20
   */
   ...
   * @param _xerc20 The address of the xsuperc20 that you want to deploy a lockbox for
  ...
  function _deployXERC20(
    ...
    for (uint256 _i; _i < _bridgesLength; ++_i) {
      XSuperchainERC20(_xerc20).setLimits(_bridges[_i], _minterLimits[_i], _burnerLimits[_i]);
    }

    XSuperchainERC20(_xerc20).transferOwnership(msg.sender);
  }
  ...
  function _deployLockbox(
    ...
    XSuperchainERC20(_xerc20).setLockbox(address(_lockbox));
