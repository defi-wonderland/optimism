...
contract InteropSmokeTest is Script {
    ...
    function sendSuperchainWETH_FFI() public returns (Identifier memory identifier, bytes memory payload) {
        ...
        bytes memory timestampResult = _executeCastBlock(log.blockNumber, "timestamp", L2_1_RPC);
    ...
    function unwrapSuperchainWETH_FFI() public {
        ...
        // Execute the unwrap
        _executeCastSend(
            Predeploys.SUPERCHAIN_WETH, vm.toString(abi.encodeCall(ISuperchainWETH.withdraw, (VALUE))), L2_2_RPC
        );
        ...
        returns (bytes32 withdrawalHash, Types.WithdrawalTransaction memory withdrawalTx)
    {
        ...
        _executeCastSend(
            ...
            vm.toString(abi.encodeCall(IL2ToL1MessagePasser.initiateWithdrawal, (DEPLOYER, 200_000, ""))),
            L2_2_RPC,
            VALUE
        );
        ...
        VmSafe.EthGetLogs[] memory logs =
            vm.eth_getLogs(beforeBlockNumber, block.number, Predeploys.L2_TO_L1_MESSAGE_PASSER, new bytes32[](0));
        ...
        address sender = address(uint160(uint256(log.topics[2])));
        address target = address(uint160(uint256(log.topics[3])));

        // Decode the non-indexed parameters from the data field
        (uint256 value, uint256 gasLimit, bytes memory data, bytes32 wHash) =
    ...
    /// @notice Tests that relaying an invalid message fails as expected
    /// @param _identifier The valid identifier from the previous successful relay
    /// @param _payload The valid payload from the previous successful relay
    function testInvalidMessageRelay_FFI(Identifier memory _identifier, bytes memory _payload) public {
        console.log("Testing invalid message relay...");

        // Create fork of destination chain
        vm.createSelectFork(L2_2_RPC);

        // Create an invalid identifier by modifying the valid one
        Identifier memory invalidIdentifier = Identifier({
            origin: _identifier.origin,
            blockNumber: _identifier.blockNumber + 1000, // Use a future block number that doesn't exist
            logIndex: _identifier.logIndex,
            timestamp: _identifier.timestamp,
            chainId: _identifier.chainId
        });

        // Create an invalid payload by modifying the nonce in the valid payload
        bytes memory invalidPayload = this.decodeSentMessagePayload(_payload);
        console.log("Created invalid payload by modifying message nonce");

        // Try to relay the invalid message with async flag
        bytes memory result = _executeCastSend(
            Predeploys.L2_TO_L2_CROSS_DOMAIN_MESSENGER,
            vm.toString(
                abi.encodeCall(
                    IL2ToL2CrossDomainMessenger(Predeploys.L2_TO_L2_CROSS_DOMAIN_MESSENGER).relayMessage,
                    (invalidIdentifier, invalidPayload)
                )
            ),
            L2_2_RPC,
            0,
            true // Set async flag to true
        );

        // Sleep for 1 second to ensure transaction has time to fail
        vm.sleep(5_000);

        // Parse the transaction hash from the result
        string memory txHash = vm.toString(result);
        console.log("Got transaction hash:", txHash);

        // Try to get the transaction receipt - should fail
        ...
        VmSafe.FfiResult memory receiptResult = vm.tryFfi(cmds);

        // Verify that the transaction was not found (should have non-zero exit code)
        require(
            receiptResult.exitCode != 0,
            "InteropSmokeTest: Invalid message relay should have failed but transaction was found"
        );

        console.log("Invalid message relay test passed - transaction correctly failed");
    }

    /// @notice Helper function to decode a SentMessage event payload and increment nonce
    /// @param _payload The raw event payload bytes to decode
    /// @return encodedPayload The re-encoded payload with incremented nonce
    function decodeSentMessagePayload(bytes calldata _payload) public pure returns (bytes memory encodedPayload) {
        // Validate the event selector
        bytes32 selector = abi.decode(_payload[:32], (bytes32));
        require(selector == SENT_MESSAGE_EVENT_SELECTOR, "Invalid event selector");

        // Decode the topics (destination, target, nonce)
        (uint256 destination, address target, uint256 nonce) = abi.decode(_payload[32:128], (uint256, address, uint256));

        // Decode the data fields (sender, message)
        (address sender, bytes memory message) = abi.decode(_payload[128:], (address, bytes));

        // Re-encode the payload with incremented nonce
        encodedPayload =
            abi.encodePacked(abi.encode(selector, destination, target, nonce + 1), abi.encode(sender, message));
    }
}
